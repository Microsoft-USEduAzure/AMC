...
if [[ -z "$password" ]]; then
	echo "Your database login will be sqladmin and you'll need a password for this login?"
        echo "Password must meet requirements for Microsoft, including capilatization, special characters.  Example: SQLAdm1nt3st1ng"
...
if [[ -z "$holname" ]]; then
	echo "Time to choose the naming convention for your servers and other resources!"
	echo "Choose a 3-5 letter with number, lower case convention, initial or acronyms work great!  Exmaple: mdw1"
	read holname
	[[ "${holname:?}" ]]
fi
...
echo "Create NSG for new subnet"
az network nsg create --name $vnet"-data-nsg" --resource-group $mgmtgrp --location $zone 
...
#  --vnet-name $vnet  \
#  --name $snet \
#  --resource-group $mgmtgrp \
  #--route-table rwb-data-udr
...
then
  ...
  sqlcmd -U $adminlogin -S "${servername}.database.windows.net" -P "$spassword" -d AMC_Synthea_Staging -i "syntheaDBwData.sql"
  ...
  sqlcmd -U $adminlogin -S "${servername}.database.windows.net" -P "$spassword" -d AMC_Synthea_Staging -i "syntheaDBschema.sql"
